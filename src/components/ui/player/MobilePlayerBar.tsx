...
export default function MobilePlayerBar() {
  ...
  const expandCoverGlowRef = useRef<HTMLDivElement | null>(null);
  const expandPlayBtnRef = useRef<HTMLButtonElement | null>(null);
  const expandAmbientBgRef = useRef<HTMLDivElement | null>(null);
  ...
  // =========================================================================
  // NAMIDA PRE-COMPUTED WAVEFORM & BEAT MAP ENGINE
  // =========================================================================
  const waveformCacheRef = useRef<Float32Array | null>(null);
  const currentTrackIdRef = useRef<string | null>(null);

  // Bước 1: Trích xuất và nén biên độ toàn bộ bài hát 1 lần duy nhất
  useEffect(() => {
    if (!currentTrack?.audio_url || currentTrack.id === currentTrackIdRef.current) return;
    currentTrackIdRef.current = currentTrack.id;
    waveformCacheRef.current = null;

    const extractWaveform = async () => {
      try {
        const response = await fetch(currentTrack.audio_url);
        ...
        const step = Math.floor(sampleRate * 0.05); // 50ms per bucket (Chuẩn Namida)
        const totalBuckets = Math.floor(channelData.length / step);
        const extracted = new Float32Array(totalBuckets);
        ...
        waveformCacheRef.current = extracted;
      } catch {
        // Fallback tự sinh beat map ảo nếu gặp lỗi network
        waveformCacheRef.current = null;
      }
    };
  ...
  // Bước 2: Tra cứu biên độ O(1) theo mili-giây và chạy hiệu ứng mượt 60 FPS
  useEffect(() => {
    ...
    const namidaLiveLoop = () => {
      ...
      if (!isDraggingSeekerRef.current) {
        ...
        if (expandedCurrentTimeTextRef.current && floorSec !== lastLoggedSecRef.current) {
          lastLoggedSecRef.current = floorSec;
          expandedCurrentTimeTextRef.current.textContent = formatTime(liveSec);
        }
      }

      // Thuật toán Namida: Tra cứu trực tiếp mảng biên độ theo bước 50ms
      let rawAmplitude = 0;
      if (waveformCacheRef.current) {
        const bucketIndex = Math.floor(liveSec / 0.05);
        if (bucketIndex >= 0 && bucketIndex < waveformCacheRef.current.length) {
          rawAmplitude = waveformCacheRef.current[bucketIndex];
        }
      } else {
        // Fallback nhịp trống mô phỏng nếu file chưa decode xong
        const beatTime = (liveSec * 2.1) % 1;
        rawAmplitude = Math.max(0, 1 - beatTime * 4);
      }

      // Giới hạn biên độ nảy an toàn (Max 1.035)
      const kickScale = 1.0 + Math.min(0.035, rawAmplitude * 0.035);
      const glowOpacity = 0.15 + rawAmplitude * 0.45;
      const kickDelta = kickScale - 1.0;

      // Render trực tiếp GPU (Không Repaint)
      ...
      if (miniGlowBackdropRef.current) {
        miniGlowBackdropRef.current.style.opacity = `${glowOpacity}`;
        miniGlowBackdropRef.current.style.transform = `scale3d(${1 + kickDelta * 1.5}, ${1 + kickDelta * 1.5}, 1)`;
      }

      if (miniCoverRef.current) {
        miniCoverRef.current.style.transform = `scale3d(${kickScale}, ${kickScale}, 1)`;
      }

      if (miniPlayBtnRef.current) {
        miniPlayBtnRef.current.style.transform = `scale3d(${1 + kickDelta * 1.2}, ${1 + kickDelta * 1.2}, 1)`;
      }

      if (expandCoverRef.current) {
        expandCoverRef.current.style.transform = `scale3d(${kickScale}, ${kickScale}, 1)`;
      }

      if (expandCoverGlowRef.current) {
        expandCoverGlowRef.current.style.opacity = `${glowOpacity * 1.2}`;
        expandCoverGlowRef.current.style.transform = `scale3d(${1 + kickDelta * 2.5}, ${1 + kickDelta * 2.5}, 1)`;
      }

      if (expandPlayBtnRef.current) {
        expandPlayBtnRef.current.style.transform = `scale3d(${1 + kickDelta * 1.5}, ${1 + kickDelta * 1.5}, 1)`;
      }

      if (expandAmbientBgRef.current) {
        expandAmbientBgRef.current.style.opacity = `${0.18 + glowOpacity * 0.25}`;
      }

      if (expandLaserBorderRef.current) {
        expandLaserBorderRef.current.style.opacity = `${0.1 + glowOpacity * 0.5}`;
      }

      rafIdRef.current = requestAnimationFrame(namidaLiveLoop);
    };
  ...
  }, [isPlaying, activeZone, currentTimeRef, audioRef, swipeOffsetX]);
      ...
      {isExpanded && (
        ...
        >
          <div
            ...
            style={{
              backgroundImage: 'radial-gradient(circle at 50% 45%, rgba(255, 60, 60, 0.45) 0%, rgba(255, 60, 60, 0.12) 45%, transparent 75%)',
            }}
          />
              ...
              <div className="w-full flex flex-col items-center justify-center h-full animate-fadeIn relative overflow-visible">
                <div className="relative flex items-center justify-center mb-6 flex-shrink-0">
                  <div
                    ...
                    className="absolute inset-0 w-[72vw] max-w-[260px] aspect-square rounded-3xl bg-red-600/40 blur-2xl pointer-events-none will-change-transform"
                  />
                  <div
                    ref={expandCoverRef}
                    className="relative w-[72vw] max-w-[260px] aspect-square rounded-3xl overflow-hidden bg-zinc-950 flex items-center justify-center border border-white/20 will-change-transform shadow-2xl z-10"
                  >
                    {currentAlbum?.cover_url ? (
                      <img
                        src={currentAlbum.cover_url}
                    ...
                    {parsedLyrics.map((line, idx) => {
                      ...
                      return (
                        <p
                          ...
                          className={`text-[17px] leading-relaxed font-bold tracking-tight text-left select-none cursor-pointer transition-colors duration-200 ${
                            isActive
